...
import { IonAlert, IonBackButton, IonButton, IonButtons, IonContent, IonHeader, IonIcon, IonItem, IonLabel, IonList, IonListHeader, IonNote, IonPage, IonRange, IonSelect, IonSelectOption, IonText, IonTitle, IonToggle, IonToolbar } from '@ionic/vue';
import { addCircle, bugOutline, closeCircle, createOutline, logOutOutline, mailOutline, trashOutline } from 'ionicons/icons';
    ...
    <ion-content>
      <div class="wrapper" @click="resetDevModeCount()">
        <ion-list>
          ...
          <ion-item>
            <ion-select :label="$t('SETTINGS_PAGE.LANGUAGE')" v-model="settings.lang" @ionChange="settingsChanged()">
              <ion-select-option v-for="[code, name] in langs" :key="code" :value="code">{{ name }}</ion-select-option>
            </ion-select>
          </ion-item>
          <ion-item>
            <ion-select :label="$t('SETTINGS_PAGE.THEME')" v-model="settings.theme" @ionChange="settingsChanged()">
          ...
          <template v-if="false">
            <ion-item>
              <ion-select :label="$t('SETTINGS_PAGE.FONT')" v-model="settings.font" @ionChange="settingsChanged()">
            ...
            <ion-item>
              <ion-range :min="12" :max="20" :step="4" v-model="settings.fontSize" @ionChange="settingsChanged()" snaps>
            ...
            <ion-item>
              <ion-range :min="1" :max="2" :step="0.5" v-model="settings.lineHeight" @ionChange="settingsChanged()"
                snaps>
        ...
        <ion-list>
          ...
          <div role="button" class="big-button" @click="resetUser()">
            <div class="big-button-label"><ion-icon aria-hidden="true" :icon="createOutline"></ion-icon>{{
              $t("SETTINGS_PAGE.RESET_USER") }}</div>
        ...
        <ion-list v-if="info && info.version">
          ...
          <ion-item>
            <ion-toggle v-model="settings.isUserOptIn" @ionChange="settingsChanged()">{{
              $t("SETTINGS_PAGE.DATA_COLLECTION") }}</ion-toggle>
          </ion-item>
        </ion-list>
        <ion-list v-if="settings.devMode">
          ...
          <ion-item>
            <ion-toggle v-model="settings.devMode" @ionChange="disableDevMode($event)">{{ $t("SETTINGS_PAGE.DEV_MODE")
            }}</ion-toggle>
          </ion-item>
        </ion-list>
        <ion-list>
          ...
          <ion-item
            :href="'mailto:ill-epoc-contact@inria.fr?subject=' + encodeURIComponent($t('SETTINGS_PAGE.BECOME_PARTNER'))"
            :title="$t('SETTINGS_PAGE.BECOME_PARTNER')" target="_blank">
        ...
        <ion-list>
          ...
          <template
            v-if="libraryStore.getCustomCollections && (Array.isArray(libraryStore.getCustomCollections) ? libraryStore.getCustomCollections.length > 0 : Object.keys(libraryStore.getCustomCollections).length > 0)">
            <ion-item v-for="(collectionItem, key) in libraryStore.getCustomCollections" :key="key">
              <ion-label>
                <ion-text>{{ collectionItem.title }}</ion-text>
                <p style="white-space: normal">{{ collectionItem.url }}</p>
              </ion-label>
              <ion-icon :icon="closeCircle" color="inria" slot="end"
                @click="deleteCollection($event, 'WIP')"></ion-icon> <!-- TODO: replace 'WIP' with collection ID -->
            </ion-item>
          </template>
          ...
          <ion-alert trigger="present-library-prompt" :header="$t('SETTINGS_PAGE.ADDING_LIBRARY')"
            :buttons="libraryPromptButtons" :inputs="libraryPromptInputs"></ion-alert>
        </ion-list>
        <ion-list>
          <ion-list-header>
          ...
          <div role="button" class="big-button" @click="deleteData()">
            <div class="big-button-label"><ion-icon aria-hidden="true" :icon="trashOutline"></ion-icon>{{
              $t("SETTINGS_PAGE.DELETE_DATA") }}</div>

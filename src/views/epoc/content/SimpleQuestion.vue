...
const userAssessment = computed(() =>
    reading.value ? reading.value.assessments.find((a) => a.id === props.content.id) : undefined
);
...
const checkAnswer = (event: Event) => {
    ...
    readingStore.saveStatement(
        props.epocId,
        'questions',
        (props.content as SimpleQuestion).question,
        'attempted',
        true
    );
...
<template>
    ...
    >
        <ion-button
            v-if="!questionAnswered && question.responses.length > 0"

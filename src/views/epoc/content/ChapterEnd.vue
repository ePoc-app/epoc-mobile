...
<template>
  <template v-if="nextChapter">
    <p><b class="ion-text-capitalize"> {{$t('PLAYER.CHAPTER_END.FOLLOWING_NEXT')}}:</b> {{nextChapter.title}} </p>
    <ion-button expand="block" size="large" color="outline-button" fill="outline" :router-link="'/epoc/toc/'+epoc.id">
      <span>{{$t('FLOATING_MENU.TOC')}}</span>
    </ion-button>
    <ion-button expand="block" size="large" color="inria" :router-link="'/epoc/play/'+epoc.id+'/'+nextChapter.id">
